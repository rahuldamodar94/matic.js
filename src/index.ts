...
export default class Matic extends SDKClient {
  ...
  startWithdrawMintableERC721(token: address, amount: BN | string, options?: SendOptions) {
    this._validateInputs(token, amount, options)
    return this.withdrawManager.burnMintableERC721Tokens(token, amount, options)
  }
  ...
  withdrawMintableERC721Token(txHash: string, predicate: address, options?: SendOptions) {
    if (!txHash) {
      throw new Error(`txHash not provided`)
    }
    if (options && !options.from) {
    ...
    return this.withdrawManager.startExitForMintableERC721BurntToken(txHash, predicate, options)
  }
